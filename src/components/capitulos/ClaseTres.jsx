...
const ClaseTres = () => {
  return (
      ...
      <Card className="mb-2">
        <Card.Header className='apuntes-react'><Badge pill bg="dark">Clase Nº 3 </Badge><h1>Estructura de archivos, CSS, Bootstrap, GoogleFonts & GitHub</h1></Card.Header>
        <Card.Body>
          ...
          <Card.Text>
            ...
            <p>Bajando la extension 'ES7 React/Reduxs/React Native Sn' podemos hacer uso del atajo <code>rafce</code> podemos crear de manera automatica la estructura basica de un componente.</p>
          ...
          <Card.Text>
            <ol>
              <li>Dentro de mi carpeta 'src' creo mi archivo de SASS con el nombre 'style.scss'.</li>
              <li>Ahora necesitamos importar el archivo 'style.scss' dentro de nuestro componente 'App.js'.</li>
              <li>En caso de no tener instalada la libreria SASS vamos a ver un mensaje de error en pantalla "Module error, cannot find Module Sass" para solucionar este error procedemos a instalar SASS. Para instalarlo frenamos la consola y ejecutamos el siguiente codigo 'npm install sass'.</li>
              <li>Ya podemos crear estilos CSS dividos en 'modulos' usando SASS. Para hacerlo podemos crear una carpeta 'styles' y dentro de ella guardamos nuestros modulos colocando siempre un guion-bajo como primer letra dentro del nombre de cada modulo, por ejemplo '_header.scss'.</li>
              <li>Para poder ver los cambios de estilo que vamos haciendo tenemos que importar los modulos dentro de la hoja principal, nuestro 'archivo global' que se llama 'styles.scss', por ejemplo para importar nuestro modulo '_header.scss' alojado en la carpeta 'styles' tenemos que añadir la siguiente linea <code>@import 'styles/header'; </code> al archivo SASS principal, en este caso 'Styles.scss'</li>
            </ol>
          </Card.Text>
          ...
          <Card.Text>
            ...
            <ul>
              ...
              <li> <code> ' git init ' </code> inicializamos un repositorio local</li>
              <li>1-1 <code> ' git add . ' </code> Sube los archivos al staging area</li>
              <ul>
                <li>1-1-1 <code> ' git reset . ' </code> Saca lo que subimos al staging usando el comando 'git add.'</li>
                <li>1-1-2 <code> ' git status ' </code> Visualizo el estado de mis archivos</li>
              </ul>
              <li>1-2 <code>' git commit -m aca-dejo-un-mensaje ' </code>  Hago un commit de los cambio que hice al repositorio local y dejo un mensaje</li>
              <ul>
                <li>1-2-1 <code> ' git log ' </code> Veo los commits realizados con los datos de: autor, lahora y el mensaje</li>
                <li>1-2-2 <code>' git log --oneline' </code>  Visualizo los commits en una sola linea sin el horario ni el autor</li>
                <li>1-2-3 <code> ' git commit --amend ' </code> Modifico el mensaje del ultimo commit. Para escribir el nuevo mensaje toco 'i'. Para confirmar y salir tipeo 'esc', ':', 'w' y 'q'</li>
              </ul>
            </ul>

            <ul>
              <li>2-1 <code>' git push -u origin Nombre-para-identificar-la-rama ' </code>  Hacemos un push de una rama nueva (cuando estamos trabajando en el master o una rama haciendo commits constantemente, no es necesario escribir esta sentencia completa, con escribir - git push - es suficiente, ya que el primer commit nos posiciona en destino para los siguientes commits)</li>
              <li>2-2 <code> ' git checkout nombre-de-la-rama-a-la-que-quiero-ir ' </code> Me muevo a una rama distinta de la que estoy</li>
              <ul>
                <li> <code> ' git checkout -- . ' </code> Todos mis archivos que se encuentran en mi repositorio van a ser reconstruidos como se encontraban en el ultimo commit</li>
              </ul>
            </ul>

          <ul>
            <li>3-1 - <code> ' git branch -l ' </code> puedo ver mi lista de ramas dentro de git, con la tecla 'Q' vuelvo a la consola</li>
            <ul>
              <li>3-1-1  <code>' git branch nombre-de-la-nueva-rama '</code>  Crea una rama nueva con el nombre 'nombre-de-la-nueva-rama'</li>
              <li>3-1-2  <code>' git branch -D nombre-de-la-rama-que-quiero-borrar'</code> con este comando puedo eliminar una rama </li>
              <li>3-1-3  <code>' git branch -m '</code>  asigna un nuevo nombre a la rama en la que estas ubicado</li>
            </ul>
          </ul>

          <ul>
            <li>4-1 -  <code>' git checkout -b Nombre-de-nueva-rama '</code>  este comando sirve para crear una nueva rama y movernos a ella</li>
          </ul>

          <ul>
            <li>5-1 -  <code>' git switch - '</code> este comando me lleva a la rama anterior en la que estaba trabajando</li>
            <ul>
              <li>5-1-1  <code>' git switch nombre-de-la-rama-a-la-que-quiero-ir'</code> me lleva a una rama especifica</li>
            </ul>
          </ul>

          <ul>
            <li>6-1 -  <code>' git pull '</code> Este comando me trae lo archivos actualizados a mi proyecto local en caso que hallan sido editados dentro del repositorio.</li>
          </ul>
  
          <ul><li>7-1  <code>' git merge nombre-de-la-rama '</code> combina la rama en la que estoyubicado con la que coloque en el merge. </li></ul>
          <ul>
            <li>Comandos peligrosos</li>
            <li>8-1  <code>' git reset --soft nombre-de-la-rama o commit '</code> vuelve al punto que le indiquemos pero mantiene los archivos tal cual ...</li>
            <li>9-1  <code>' git reset --hard nombre-de-la-rama o commit '</code> vuelve al punto que le indicamos borrando todo lo que paso luego de ese commit que le indicamos</li>
          </ul>

          GITHUB<br/>
          <ul>
            <li> <code>' git remote add origin URL '</code> conecta el repo local con el repo remoto</li>
            <li> <code>' git push -u origin nombre-de-la-rama'</code> sube la rama a Github</li>
            <li> <code>' git pull '</code> nos trae toda la rama a nuestro proyecto, desde el repo en la nube al local. Hace un 'git fetch' y un 'git merge' al mismo tiempo.</li>
            <li> <code>' git clone URL '</code> clona el repo de la nube a nuestra computadora.</li>
            <li> <code>' git clone URL -b nombre-de-la-rama -single-branch'</code> clona solo la rama que le indicamos en nuestra computadora.</li>
            <hr />
